...
exports.LiveObjectManager = void 0;
 ...
 */
class LiveObjectManager extends common_utils_1.TypedEventEmitter {
    /**
     ...
     */
    constructor(_liveRuntime, _containerRuntime) {
        super();
     ...
     */
    registerObject(id, runtime, initialState, handlers) {
        return __awaiter(this, void 0, void 0, function* () {
            // Get/create containers synchronizer
            if (!this._synchronizer) {
                this._synchronizer = new _1.ContainerSynchronizer(runtime, this._containerRuntime, this._liveRuntime, this);
            }
            // Register object
            this._synchronizer.registerObject(id, handlers);
            const initialEvent = {
                clientId: yield (0, _1.waitUntilConnected)(runtime),
                timestamp: 0,
                data: initialState,
                name: _1.ObjectSynchronizerEvents.update,
            };
     ...
     */
    getLatestEventForObject(objectId) {
        var _a;
        return (_a = this.getEventsForObject(objectId)) === null || _a === void 0 ? void 0 : _a[0];
    }
    /**
     ...
     * @returns the latest event sent, or undefined if there is none
     */
    getLatestEventForObjectClient(objectId, clientId) {
        var _a;
        return (_a = this.objectStoreMap.get(objectId)) === null || _a === void 0 ? void 0 : _a.get(clientId);
    }
    /**
    ...
    getEventsForObject(objectId) {
        ...
        if (!clientMap)
            return undefined;
        return (_a = [...clientMap.values()]) === null || _a === void 0 ? void 0 : _a.sort((a, b) => b.timestamp - a.timestamp);
    }
    /**
    ...
    sendEventForObject(objectId, data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const valueSent = yield this._synchronizer.sendEventForObject(objectId, data);
    ...
    sendThrottledEventForObject(objectId, data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const valueSent = yield this._synchronizer.sendThrottledEventForObject(objectId, data);
            this.updateEventLocallyInStore(objectId, valueSent);
            return valueSent;
        });
    }
     ...
     */
    clientIdDidChange(originalClientId, newClientId) {
    ...
    __dangerouslySetContainerRuntime(cRuntime) {
        ...
        this.startReceivingSignalUpdates();
        (_a = this._synchronizer) === null || _a === void 0 ? void 0 : _a.__dangerouslySetContainerRuntime(cRuntime);
    }
    /**
     * Set the audience
     */
    ...
    onReceivedSignal(message, local) {
        var _a;
        if (local ||
            !message.clientId ||
            !(0, _1.isILiveEvent)(message.content) ||
            typeof message.content.data !== "object")
            return;
        this.dispatchUpdates(_1.ObjectSynchronizerEvents.update, message.clientId, message.content.data, local);
        // If the non-local user is connecting for the first time
        if (message.type === _1.ObjectSynchronizerEvents.connect) {
            (_a = this._synchronizer) === null || _a === void 0 ? void 0 : _a.onSendBackgroundUpdates();
        }
    }
    dispatchUpdates(type, senderId, updates, local) {
        for (const id in updates) {
            const data = updates[id];
            const receivedEvent = {
                clientId: senderId,
                timestamp: data.timestamp,
                data: (0, _1.cloneValue)(data.data),
                name: type,
            };
            ...
            if (!didUpdate)
                continue;
            // if (local) return;
            this.emit(_1.ObjectSynchronizerEvents.update, id, (0, _1.cloneValue)(receivedEvent), local);
        }
    }
    /**
     * @hidden
     ...
     */
    updateEventLocallyInStore(objectId, event) {
        let clientMap = this.objectStoreMap.get(objectId);
        if (clientMap) {
            const existingEvent = clientMap.get(event.clientId);
            if (existingEvent) {
                // We already have an event for this user, so we update it if it is newer
                if (!LiveEvent_1.LiveEvent.isNewer(existingEvent, event))
                    return false;
            }
    ...
    stopReceivingSignalUpdates() {
        if (!this._onBoundReceivedSignalListener)
            return;
        this._containerRuntime.off("signal", this._onBoundReceivedSignalListener);
    }
}
exports.LiveObjectManager = LiveObjectManager;
//# sourceMappingURL=LiveObjectManager.js.map